...
func (r *PlaylistRepo) Create(ctx context.Context, userID uuid.UUID, name string, parentID *uuid.UUID, isFolder bool, position int, imported bool) (*domain.Playlist, error) {
	...
	query := `INSERT INTO playlists (user_id, name, parent_id, is_folder, position, imported)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING playlist_id, name, parent_id, is_folder, position, imported, created_at`
...
func (r *PlaylistRepo) GetTree(ctx context.Context, userID uuid.UUID) ([]domain.Playlist, error) {
	query := `SELECT playlist_id, parent_id, name, is_folder, position, imported, created_at
	...
	for rows.Next() {
		var p domain.Playlist
		if err := rows.Scan(&p.PlaylistID, &p.ParentID, &p.Name, &p.IsFolder, &p.Position, &p.Imported, &p.CreatedAt); err != nil {
			continue
		}
		p.Children = []domain.Playlist{}
		p.Tracks = []domain.Track{}
...
func (r *PlaylistRepo) Get(ctx context.Context, userID, playlistID uuid.UUID) (*domain.Playlist, error) {
	var p domain.Playlist
	query := `SELECT playlist_id, parent_id, name, is_folder, position, imported, created_at FROM playlists
	WHERE user_id = $1 AND playlist_id = $2`
	err := r.pool.QueryRow(ctx, query, userID, playlistID).Scan(&p.PlaylistID, &p.ParentID, &p.Name, &p.IsFolder, &p.Position, &p.Imported, &p.CreatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *PlaylistRepo) DeleteImportedForUser(ctx context.Context, userID uuid.UUID) error {
	_, err := r.pool.Exec(ctx, `DELETE FROM playlists WHERE user_id = $1 AND imported = true`, userID)
	return err
}
